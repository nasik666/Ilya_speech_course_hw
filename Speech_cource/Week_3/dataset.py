...
class Dataset(torch_data.Dataset):
    """
    Loads audio files from a directory organized by speaker.
    Each subdirectory is treated as one speaker identity.
    Features are extracted on-the-fly using the provided `feats` module.
    """
    ...
    def speakers(self) -> int:
        # number of unique speakers = last assigned index + 1
        return self._speakers[-1] + 1


def collate_fn(batch) -> tuple[torch.Tensor, torch.Tensor, list[str]]:
    """
    Pads feature tensors to the same length within a batch.
    Shorter sequences are zero-padded on the right.
    Returns: (padded features, speaker labels, file paths)
    """
